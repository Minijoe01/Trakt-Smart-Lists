...
def format_minutes(minutes):
    if not minutes or minutes != minutes or minutes <= 0:   # minutes != minutes = NaN
        return "inconnue"
    minutes = int(round(minutes))   # les durées arrivent parfois en float (110.0 après concat pandas)
    h = minutes // 60
    m = minutes % 60
...
<style>
    ...
    div[data-testid="stProgressBarTrack"] > div { background: var(--am-green) !important; }   /* #00A392 — choix de Cédric */
    ...
    .progress-low, .progress-mid, .progress-high { background: var(--am-green); }   /* #00A392 — choix de Cédric */
...
def oublier_connexion():
    # La révocation côté Trakt tue le refresh token : même si l'effacement du
    # cookie capotait, la reconnexion auto échouerait sur ce token mort et
    # réessaierait l'effacement à chaque visite. Pas de cookie supplémentaire :
    # une seule opération par passage, comme la version éprouvée.
...
def recuperer_last_activity(at):
    """1 SEUL appel : horodatages des dernières activités Trakt
    (officiel : GET /sync/last_activities). Sert à détecter 'rien n'a changé'."""
    try:
        r = requests.get("https://api.trakt.tv/sync/last_activities", headers=entetes(at), timeout=10)
        return r.json() if r.status_code == 200 else None
    except Exception:
        return None

def last_activity_snapshot():
    """Dernier état connu (session, sinon disque à côté du cache historique)."""
    snap = st.session_state.get("_last_activities")
    if snap is not None:
        return snap
    try:
        pseudo = st.session_state.get("infos", {}).get("pseudo", "")
        with open(_hist_cache_chemin(pseudo), "r", encoding="utf-8") as f:
            return json.load(f).get("last_activities")
    except Exception:
        return None

def last_activity_enregistrer(snap):
    """Fige l'état APRÈS analyse (session + disque, en préservant l'historique)."""
    st.session_state["_last_activities"] = snap
    try:
        pseudo = st.session_state.get("infos", {}).get("pseudo", "")
        try:
            with open(_hist_cache_chemin(pseudo), "r", encoding="utf-8") as f:
            ...
            d = {"v": _HIST_CACHE_VERSION, "histo": {}}
        d["last_activities"] = snap
        d.setdefault("v", _HIST_CACHE_VERSION)
...
def dropped_cache_lire(pseudo):
    """Ids Trakt des séries officiellement abandonnées (fonction 'Drop', section
    cachée dropped), lus à côté du cache progressions. None = jamais chargés,
    à distinguer de la liste vide (= aucune série abandonnée)."""
    if not pseudo:
        return None
    try:
        with open(_prog_cache_chemin(pseudo), "r", encoding="utf-8") as f:
        ...
        if ab is None:
            return None
        return {int(x) for x in ab}, (d.get("dropped_det") or {})
    except Exception:
        return None

def dropped_cache_ecrire(pseudo, ids, det):
    if not pseudo:
        return
    try:
        try:
            ...
            d = {}
        if d.get("v") != _PROG_CACHE_VERSION:
            d = {"v": _PROG_CACHE_VERSION, "prog": d.get("prog") if isinstance(d.get("prog"), dict) else {}}
        d["dropped"] = sorted({int(x) for x in ids})
        d["dropped_det"] = {str(k): v for k, v in (det or {}).items()}
        d["dropped_maj"] = datetime.now(timezone.utc).isoformat()
        with open(_prog_cache_chemin(pseudo), "w", encoding="utf-8") as f:
            json.dump(d, f, ensure_ascii=False)
    except Exception:
        pass
...
def _progressions_source(pseudo, h):
    ...
    ab = dropped_cache_lire(pseudo)
    if data:
        if "dropped" not in data and ab is not None:
            data = dict(data)
            data["dropped"], data["dropped_det"] = ab[0], ab[1]
        return data
    prog = prog_cache_lire(pseudo)
    if not prog:
        return None
    vus, der, _ = _eps_vus_der(h)
    out = {"prog": prog, "vus": vus, "der": der}
    if ab is not None:
        out["dropped"], out["dropped_det"] = ab[0], ab[1]
    return out
...
def recuperer_progressions(at, histo, pseudo, forcer=False):
    ...
    _ab = recuperer_abandons_officiels(at)   # 1 seule requête, uniquement pendant un fetch opt-in
    if _ab is not None:
        dropped_cache_ecrire(pseudo, _ab[0], _ab[1])
        data["dropped"], data["dropped_det"] = sorted(_ab[0]), _ab[1]
    if _dbg:
        data["debug"] = _dbg
    return data

def recuperer_abandons_officiels(at):
    """Séries officiellement abandonnées sur Trakt = section 'dropped' des éléments
    masqués (fonction Drop lancée en 2025 : elles disparaissent du Progress/Up Next/Calendrier).
    Elles sont chez nous EXCLUES de la projection de fin : on ne refait pas un retard
    qu'on ne rattrapera jamais. 1 requête/page de 100, appelé uniquement lors d'un
    fetch progressions (bouton opt-in) — ZÉRO appel à l'affichage.
    Retourne (ids:set, det:dict sid->{titre,annee,slug,hidden_at}) ou None si indisponible."""
    h = entetes(at)
    for base in ("https://api.trakt.tv/users/me/hidden/dropped",
                 "https://api.trakt.tv/users/hidden/dropped"):   # 2e forme : la doc normalise sans /me
        try:
            ids, det, page = set(), {}, 1
            while page <= 5:   # garde-fou : 500 abandons max
                r = requests.get(base, headers=h, params={"type": "show", "page": page, "limit": 100}, timeout=15)
                if r.status_code != 200:
                    break
                lot = r.json() or []
                for it in lot:
                    ...
                    if ids0.get("trakt"):
                        sid = int(ids0["trakt"])
                        ids.add(sid)
                        det[sid] = {"titre": sh.get("title"), "annee": sh.get("year"),
                                    "slug": ids0.get("slug"), "hidden_at": it.get("hidden_at")}
                if len(lot) < 100:
                    return ids, det
                page += 1
            if ids or det:   # pagination cassée en route : on garde ce qu'on a
                return ids, det
        except Exception:
            pass
    return None
                ...
                for it in d:
                    ...
                    if tid:
                        res[(lbl, tid)] = {"note": it.get("rating", 0) or 0,
                                           ...
                                           "network": (med.get("network") or "")}   # séries : gratuit, films : vide
                p += 1
...
def lancer_analyse(rafraichir=False, page_suivante="🏠 Tableau de bord"):
    ...
    try:
        ...
        _act_fin = recuperer_last_activity(st.session_state["access_token"])  # 1 appel : on fige l'état
        if _act_fin is not None:
            last_activity_enregistrer(_act_fin)
    except Exception as e:
    ...
    if "res" in st.session_state:
        ...
        with c1:
            if st.button("🔄 Analyse rapide", use_container_width=True):
                # 🛰️ 1 SEUL appel d'abord : si Trakt dit "aucune activité depuis la dernière
                # analyse", on saute TOUT le re-téléchargement (last_activities, officiel).
                _act_now = recuperer_last_activity(st.session_state["access_token"])
                _snap = last_activity_snapshot()
                if _act_now is not None and _snap is not None and _act_now == _snap:
                    st.toast("✅ Déjà à jour : aucune activité Trakt depuis la dernière analyse — analyse sautée !", icon="🛰️")
                else:
...
def _progressions_preparer(data, utz, dropped=None):
    """À partir du cache 'progressions' : lignes prêtes à afficher.
    Retourne (actives, pauses, abandonnees, reste_proj) — PUR, ZÉRO appel API.
      · actives     : suivies (< 4 mois) ou simplement à jour
      · pauses      : muettes depuis 4 mois+ — non abandonnées officiellement,
                      elles COMPTEENT dans la projection de fin
      · abandonnees : statut 'dropped' officiel Trakt — arrêtées volontairement,
                      donc EXCLUES de la projection (on ne refait pas ce retard)
      · reste_proj  = épisodes restants (actives + pauses)"""
    actives, pauses, abandonnees = [], [], []
    reste_proj = 0
    if not data:
        return actives, pauses, abandonnees, 0
    if dropped is None:
        dropped = data.get("dropped")
    try:
        ab_off = {int(x) for x in (dropped or [])}
    except Exception:
        ab_off = set()
    det_ab = data.get("dropped_det") or {}
    ...
    for sid, e in prog.items():
        ...
        if sid_i in ab_off:                          # dropped officiel : volontairement arrêtée,
            ha = det_ab.get(str(sid_i), {}).get("hidden_at") if isinstance(det_ab, dict) else None
            row["abandonne_iso"] = ha                #  on ne refait pas ce retard -> hors projection
            abandonnees.append(row)
        elif j <= 120 or a_jour:                     # suivie (ou simplement à jour)
            actives.append(row)
            reste_proj += reste
        else:                                        # > 4 mois sans épisode : en pause (compte quand même)
            pauses.append(row)
            reste_proj += reste
    actives.sort(key=lambda r: r["jours"])           # les plus suivies en ce moment d'abord
    pauses.sort(key=lambda r: -r["jours"])           # les plus oubliées d'abord
    abandonnees.sort(key=lambda r: (r.get("abandonne_iso") or "", r["titre"]), reverse=True)
    return actives, pauses, abandonnees, reste_proj
...
def widget_rythme(utz):
    ...
    if not h or (not h.get("ep_det") and not h.get("films_det")):
        return
    # Même source que la grille des séries : session + repli sur le cache disque.
    # (Sinon après un F5 la session est vide -> "aucune série active" et plus de projection.)
    pseudo = st.session_state.get("infos", {}).get("pseudo", "")
    _, _, _abandonnees, reste_actives = _progressions_preparer(_progressions_source(pseudo, h), utz)
    ...
    with c1:
        with st.container(border=True):
            ...
            if r["projection"]:
                ...
                if _abandonnees:
                    st.caption(f"🚫 {len(_abandonnees)} série(s) officiellement abandonnée(s) (statut « dropped ») exclue(s) du calcul — affiché ici : ton VRAI retard, rien d'autre.")
...
def widget_series_en_cours(utz):
    ...
    if data is None:
        ...
        if st.button(f"📺 Charger ma progression ({len(vus0)} séries)", key="btn_prog_init", use_container_width=True):
            ...
            st.rerun()
        return
    actives, pauses, abandonnees, _ = _progressions_preparer(data, utz)
    if not actives and not pauses and not abandonnees:
        return
    st.divider()
    t_maj = ""
    try:
    ...
    _titre += f" · {len(pauses)} en pause" if pauses else ""
    _titre += f" · {len(abandonnees)} abandonnées" if abandonnees else ""
    _titre += ")"
    with st.expander(_titre, expanded=False):
        ...
        if _dbg:
            st.caption(f"🛰️ Dernière actualisation : **{_dbg['chargees']}/{_dbg['verifiees']}** série(s) mises à jour · affiches trouvées : **{_dbg['avec_tmdb']}**"
                       + (f" · ⚠️ {_dbg['err']}" if _dbg.get("err") else ""))
        if affiches and actives:
            _nb_tmdb = sum(1 for r in actives[:12] if r.get("tmdb"))
            if _nb_tmdb == 0:
                st.caption("⚠️ Certaines fiches n'ont pas encore leur affiche : clique sur « 🔄 Mettre à jour la progression », elles seront complétées au passage.")
            elif not TMDB_KEY:
        ...
        if actives and affiches:
            ...
            for r in actives[:8]:
                ...
                st.progress(r["pct"] / 100)
    if pauses:
        with st.expander(f"⏸️ En pause depuis +4 mois ({len(pauses)})", expanded=False):
            st.caption("Rien d'abandonné officiellement — juste de l'hibernation. Elles **comptent** dans ta projection de fin. À reprendre… ou à lâcher sans culpabiliser !")
            if data.get("dropped") is None:
                st.caption("ℹ️ La distinction abandons officiels (dropped Trakt) n'est pas encore chargée : elle viendra au prochain « 🔄 Mettre à jour la progression ».")
            for r in pauses:
    ...
    if abandonnees:
        with st.expander(f"🚫 Abandonnées officiellement ({len(abandonnees)})", expanded=False):
            st.caption("Statut **« dropped » choisi sur Trakt** : arrêtées pour de bon, donc **exclues** de ta projection de fin — ton retard n'enfle pas avec des séries que tu ne reprendras jamais. Tu en reprends une ? Regarde un épisode (ou remets-la en watchlist) et Trakt la « un-drop » tout seul.")
            for r in abandonnees:
                mois = max(1, r["jours"] // 30)
                an = f" ({r['annee']})" if r.get("annee") else ""
                lien = (f' <a href="https://trakt.tv/shows/{r["slug"]}" target="_blank" '
                        f'style="color:#CEDC00; text-decoration:none; font-size:0.85em;">🔗 Trakt</a>')
                h_ab = _dt_iso(r.get("abandonne_iso"))
                quand = f" · 🚫 le {h_ab.astimezone(utz).strftime('%d/%m/%Y')}" if h_ab else ""
                st.markdown(f"📺 <b>{_esc_html(r['titre'])}</b>{an}{lien} — **{r['pct']}%** · dernier épisode vu il y a **{mois} mois**{quand} · il en reste {r['reste']} (≈ {format_duree(r['reste_min'] / 60)})", unsafe_allow_html=True)
...
# ==================================================
# 🎨 HEATMAP — ÉCHELLE UNIQUE : les cases ET la légende piochent ici, donc
# couleurs forcément identiques (fini la légende en émojis 🟩 qui ne collaient pas).
# (seuil minimum de visionnages, couleur exacte, libellé)
# ==================================================
_HM_ECHELLE = [
    (0, "rgba(157,197,191,0.10)", "0"),
    (1, "#0E7566", "1"),   # palier +50% de luminance vs l'ancien — contraste daltonien (choix de Cédric)
    (2, "#00A392", "2 à 3"),
    (4, "#CEDC00", "4 et +"),
]

def _hm_couleur(nb_vues):
    """Couleur exacte d'une case heatmap pour un nombre de visionnages du jour."""
    _col = _HM_ECHELLE[0][1]
    for _seuil, _coul, _lbl in _HM_ECHELLE:
        if nb_vues >= _seuil:
            _col = _coul
    return _col
...
def page_stats(utz):
    ...
    while _cur <= _dmax:
        ...
        _bg = _hm_couleur(_c)   # même source que la légende (_HM_ECHELLE) : jamais de décalage
    ...
    for _wk in sorted(_weeks):
        ...
        _html.append('</div>')
    _html.append('</div>')
    # Légende en VRAIES COULEURS : chaque puce reprend la couleur exacte des cases
    _html.append('<div style="display:flex; align-items:center; gap:6px; padding:10px 2px 0; font-size:12px; color:#9DC5BF; flex-wrap:wrap;"><span>Légende :</span>')
    for _seuil, _coul, _lbl in _HM_ECHELLE:
        _html.append(f'<span title="{_lbl} visionnage(s) / jour" style="width:11px; height:11px; border-radius:2px; background:{_coul}; display:inline-block; flex:none;"></span><span style="margin-right:10px;">{_lbl}</span>')
    _html.append('<span>visionnage(s) / jour</span></div>')
    _html_doc = "".join(_html)
    _fn_html = getattr(st, "html", None)
    if _fn_html is not None:
        try:
            _fn_html(_html_doc, height=160)  # HTML brut : infobulles (title) garanties + légende en couleurs
        except Exception:
    ...
    st.caption(f"📅 du {_start.strftime('%d/%m/%Y')} au {_dmax.strftime('%d/%m/%Y')} · {_j_actifs} jour(s) de visionnage · record : {_rec_jour}/jour")
    ...
    # ==================================================================
    # 🏢 TES STUDIOS PRÉFÉRÉS (séries) — le `network` est déjà dans ton
    # historique : 0 appel API. C'est TON vrai gage de qualité, pas une
    # plateforme. Films exclus : Trakt n'expose leur studio que via 1 appel/film.
    # ==================================================================
    if tc in ["Tous", "Séries"] and not eps.empty and "network" in eps.columns:
        _es = eps.copy()
        _es["date_dt"] = pd.to_datetime(_es["date"], utc=True).dt.tz_convert(utz)
        if periode == "Période personnalisée" and toutes_dates:
            _es = _es[(_es["date_dt"] >= d_deb) & (_es["date_dt"] <= d_fin)]
        else:
            _es = appliquer_filtres_periode(_es, mt, periode)
        if genre != "Tous":
            _es = _es[_es["genre"].str.contains(genre, na=False)]
        _es = _es[_es["network"].fillna("Inconnu") != "Inconnu"]
        if not _es.empty:
            # ⚠️ pandas 3 (sur Streamlit Cloud depuis le pin streamlit==1.60) :
            # GroupBy.fillna() a été SUPPRIMÉ -> AttributeError absconse ici.
            # Accumulation simple = même rendu, increvable sous pandas 2 comme 3,
            # et tolérante à une éventuelle vieille sauvegarde sans colonne 'duree'.
            _nw_min = {}
            if "duree" in _es.columns:
                for _nw, _dd in zip(_es["network"], _es["duree"]):
                    _dm = 0.0 if pd.isna(_dd) else float(_dd or 0)
                    if _dm > 0 and isinstance(_nw, str) and _nw:
                        _nw_min[_nw] = _nw_min.get(_nw, 0.0) + _dm   # minutes cumulées
            _top_nw = sorted(_nw_min.items(), key=lambda kv: -kv[1])[:6]
            if _top_nw:
                _notes_nw = {}
                for (_lb, _ti), _inf in st.session_state.get("ratings", {}).items():
                    if _lb == "Série":
                        _nwk = (_inf.get("network") or "").strip().lower()
                        if _nwk and _inf.get("note", 0) > 0:
                            _notes_nw.setdefault(_nwk, []).append(_inf["note"])
                st.divider()
                with st.container(border=True):
                    st.markdown("#### 🏢 Tes studios préférés (séries)")
                    st.caption("Heures cumulées par **network (studio/chaîne)**. Le 🔥 = « studio fétiche » : tu notes ses séries ≥ 7,5/10 en moyenne (3 séries notées minimum). Les films ne sont pas comptés : Trakt ne donne pas leur studio.")
                    _tot_nw = sum(_m for _, _m in _top_nw) / 60 or 1.0   # heures du top 6
                    for _nw_name, _min_nw in _top_nw:
                        _nw_val = _min_nw / 60   # heures
                        _pc_nw = _nw_val / _tot_nw
                        _nl = str(_nw_name).lower()
                        _moy_l = _notes_nw.get(_nl)
                        _fav = bool(_moy_l) and len(_moy_l) >= 3 and (sum(_moy_l) / len(_moy_l)) >= 7.5   # n>=3 : 2 notes = preuve fragile
                        _badge = " · 🔥 studio fétiche" if _fav else ""
                        _moy_txt = f" · ta moyenne **{sum(_moy_l) / len(_moy_l):.1f}/10** ({len(_moy_l)} notées)".replace(".", ",") if _moy_l else ""
                        st.markdown(f"**{_esc_html(str(_nw_name))}**{_badge} — {format_duree(float(_nw_val))} · **{round(_pc_nw * 100)}%** de tes heures{_moy_txt}")
                        st.progress(min(_pc_nw, 1.0))
...
def construire_profil(histo, utz):
    ...
    # --- TES studios fétiches (SÉRIES) : réseaux où ≥ 2 de TES notes perso font ≥ 7,5/10 ---
    # Donnée gratuite : le 'network' des séries est déjà dans tes notes (ratings) et ton
    # historique. Le studio des FILMS, lui, exigerait 1 appel par film -> volontairement exclu.
    notes_par_studio = {}
    for (lbl_r, _tid_r), info in st.session_state.get("ratings", {}).items():
        if lbl_r == "Série":
            nw = (info.get("network") or "").strip()
            if nw and nw != "Inconnu" and info.get("note", 0) > 0:
                notes_par_studio.setdefault(nw, []).append(info["note"])
    studios_notes = {k: (sum(v) / len(v), len(v)) for k, v in notes_par_studio.items()}
    studios_fetiches = {k.lower() for k, (moy, n) in studios_notes.items() if n >= 3 and moy >= 7.5}   # 3 notes min : évite l'effet « j'avais l'abonnement » (règle validée par Cédric)
    ...
    return {
        ...
        "studios_fetiches": studios_fetiches,
        "studios_notes": studios_notes,
...
def evaluer_contenu(item, profil, maintenant_tz):
    ...
    # 2d. Bonus "ton studio fétiche" — SÉRIES uniquement : le `network` vit déjà dans TES
    # données (le studio des films coûterait 1 appel par film -> contraire à la règle légère).
    nw_it = (med.get("network") or "").strip()
    if item["type"] != "movie" and nw_it.lower() in profil.get("studios_fetiches", set()):
        score += 4
        raisons.append(f"De TON studio fétiche {nw_it} (d'après tes notes Trakt) (+4)")
        tags.append(("🏢 Studio fétiche", f"TES notes moyennes ≥ 7,5/10 sur ≥ 3 séries {nw_it} · +4 pts"))
...
def _url_justwatch(titre):
    """Lien de RECHERCHE JustWatch : ZÉRO appel API, aucun poids dans le score —
    une plateforme n'est ni un gage ni un repoussoir, juste un plan B pratique."""
    from urllib.parse import quote
    return "https://www.justwatch.com/fr/recherche?q=" + quote(str(titre or ""))

def _ids_interdits_roulette(h, raw_wl, raw_items):
    """Contenus exclus de la pioche 🔥 Tendances : déjà vus (historique) + déjà sur
    TES listes (watchlist incluse). Tout vient des données déjà chargées : 0 appel."""
    deja = set()
    for m in (h or {}).get("films_det", []) or []:
        try: deja.add(int(m.get("id") or 0))
        except Exception: pass
    for e in (h or {}).get("ep_det", []) or []:
        try: deja.add(int(e.get("id") or 0))
        except Exception: pass
    for it in (raw_wl or []) + (raw_items or []):
        try: deja.add(int(it.get("tid") or it.get("id") or 0))
        except Exception: pass
    deja.discard(0)
    return deja

def recuperer_tendances():
    """🔥 Films + séries tendance du moment (officiel /movies|shows/trending, top 20).
    2 requêtes SEULEMENT au clic 🔥, puis 6 h de cache session. ZÉRO appel à l'affichage.
    Contenu public : la clé API suffit (pas besoin du token utilisateur)."""
    cache = st.session_state.get("_tendances_cache")
    if cache and (time.time() - cache[0]) < 6 * 3600:
        return cache[1]
    ent = {"Content-Type": "application/json", "trakt-api-version": "2", "trakt-api-key": CLIENT_ID}
    items = []
    for typ, lbl, cle in (("movies", "Film", "movie"), ("shows", "Série", "show")):
        try:
            r = requests.get(f"https://api.trakt.tv/{typ}/trending", headers=ent,
                             params={"limit": 20, "extended": "full"}, timeout=15)
            if r.status_code != 200:
                continue
            for it in r.json() or []:
                m = it.get(cle) or {}
                ...
                if not tid:
                    continue
                n_w = int(it.get("watchers") or 0)
                rt = m.get("runtime") or 0
                items.append({"type": lbl, "tid": int(tid), "titre": m.get("title") or "?",
                              "annee": m.get("year"),
                              "genres": ", ".join(g.capitalize() for g in (m.get("genres") or [])) or "Inconnu",
                              "note": round(m.get("rating") or 0, 1) or None,
                              "temps": (format_duree(rt / 60) if lbl == "Film" else (f"{rt} min/ép." if rt else "?")),
                              "duree": rt, "duree_min": rt,
                              "network": m.get("network") or "",
                              "tmdb": ids0.get("tmdb"), "slug": ids0.get("slug"),
                              "lien": f"https://trakt.tv/{'movies' if lbl == 'Film' else 'shows'}/{ids0.get('slug') or tid}",
                              "watchers": n_w,
                              "score": 50, "raisons": [], "averti": [], "friction": 50,
                              "tags": [("🔥 Tendance Trakt", f"{n_w} spectateurs cette semaine")],
                              "pas_pour_moi": False, "ajout": None, "votes": m.get("votes"),
                              "nb_episodes": m.get("aired_episodes") or 0,
                              "status": m.get("status") or "", "certification": m.get("certification")})
        except Exception:
            continue
    st.session_state["_tendances_cache"] = (time.time(), items)
    st.session_state["_trending_items"] = items
    return items

def _pool_tendances():
    """Tendances du moment hors vus/listes (utilise le cache session — 0 appel)."""
    items = st.session_state.get("_trending_items") or []
    deja = _ids_interdits_roulette(st.session_state.get("historique"),
                                   st.session_state.get("raw_wl"), st.session_state.get("raw_items"))
    return [i for i in items if i["tid"] not in deja and i["titre"] != st.session_state.get("_roulette_actuel")]
...
def page_quoi_regarder(utz):
    ...
    tous_networks = {r.get("network") for r in resultats if r.get("network")}
    tous_networks.discard("Inconnu"); tous_networks.discard("")
    ...
    with cf5:
        f_network = st.selectbox("🏢 Studio (séries)", ["Tous les studios"] + sorted(tous_networks), key="qr_network",
                                 help="Le network (studio/chaîne) de la série. Les films ne sont pas filtrables ici : Trakt ne donne pas leur studio.")
    with cf6:
    ...
    for r in resultats:
        ...
        if preset_fn is not None:
            ...
            if f_network != "Tous les studios" and (r.get("network") or "") != f_network: continue
    ...
    def _pool_roulette(mode):
        ...
        return pool0
    col_roul, col_roul2, col_roul3, col_info = st.columns([0.25, 0.27, 0.27, 0.21])
    with col_roul:
    ...
    with col_roul2:
        if st.button("🧭 Roulette découverte — sors de ta zone", use_container_width=True, key="btn_roulette_decouverte"):
            ...
            st.rerun()
    with col_roul3:
        if st.button("🔥 Tendances — le monde regarde", use_container_width=True, key="btn_roulette_tendances",
                     help="Le top films + séries du moment sur Trakt, hors vus et hors listes — la liste est mémorisée 6 h pour rester instantanée."):
            items = recuperer_tendances()
            pool = _pool_tendances() if items else []
            if pool:
                choix = random.choice(pool)
                st.session_state["_roulette"] = choix
                st.session_state["_roulette_actuel"] = choix["titre"]
                st.session_state["_roulette_mode"] = "tendances"
            else:
                st.session_state["_roulette_vide_tendances"] = True
            st.rerun()
    with col_info:
        st.caption("🎲 **Roulette** pioche dans tes filtres · 🧭 **Découverte** sort de ta zone (≥ 7,5) · 🔥 **Tendances** : ce que le monde regarde maintenant, hors vus & listes.")
    ...
    if st.session_state.pop("_roulette_vide_tendances", None):
        st.caption("🔥 Rien de neuf à piocher ces jours-ci : tu as déjà tout vu (ou tout listé) parmi les tendances du moment. Impressionnant.")
    ...
    if roul:
        with st.container(border=True):
            ...
            with cm:
                ...
                lien_html_r += f' · <a href="{_url_justwatch(roul["titre"])}" target="_blank" style="color:#9DC5BF; text-decoration:none; font-size:0.85em;">🔎 où regarder ? (JustWatch)</a>' 
                _intro_r = {"decouverte": "🧭 **L'aventure a choisi (hors zone de confort) :",
                            "tendances": "🔥 **Le monde regarde ça en ce moment :",
                            }.get(st.session_state.get("_roulette_mode"), "🎲 **Le hasard a choisi :")
                ...
                _wch = f" · 👥 dans {int(roul['watchers']):,} salons cette semaine".replace(",", " ") if roul.get("watchers") else ""
                st.caption(f"⭐ {note_r} · ⏱️ {roul['temps']} · 🎭 {roul['genres']}{ep_r} · Score {int(roul['score'])}/100{_wch}")
                ...
                if st.button("🎲 Une autre ?", key="btn_roulette_next"):
                    if st.session_state.get("_roulette_mode") == "tendances":
                        pool = _pool_tendances()
                    elif st.session_state.get("_roulette_mode") == "decouverte":
