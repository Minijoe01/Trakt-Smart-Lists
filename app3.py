...
def page_stats(utz):
    ...
    if tc in ["Tous", "Séries"] and not eps.empty:
        ...
        if not _es.empty:
            _nw_h = _es.groupby("network")["duree"].fillna(0).sum().div(60)
            _nw_h = _nw_h[_nw_h > 0].sort_values(ascending=False).head(6)
            if not _nw_h.empty:
                _notes_nw = {}
                ...
                with st.container(border=True):
                    ...
                    _tot_nw = float(_nw_h.sum()) or 1.0
                    for _nw_name, _nw_val in _nw_h.items():
                        _pc_nw = float(_nw_val) / _tot_nw
