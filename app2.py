        ...
        if not _es.empty:
            ...
            if _top_nw:
                ...
                with st.container(border=True):
                    ...
                    st.caption("Heures cumulées par **network (studio/chaîne)** — déjà dans tes données, sans un appel de plus. Le 🔥 = « studio fétiche » : tu notes ses séries ≥ 7,5/10 en moyenne (2 minimum). Les films restent dehors : leur studio coûterait 1 appel par film.")
                    ...
                    for _nw_name, _min_nw in _top_nw:
                        ...
                        _fav = bool(_moy_l) and len(_moy_l) >= 2 and (sum(_moy_l) / len(_moy_l)) >= 7.5
...
def construire_profil(histo, utz):
    ...
    studios_fetiches = {k.lower() for k, (moy, n) in studios_notes.items() if n >= 2 and moy >= 7.5}
...
def evaluer_contenu(item, profil, maintenant_tz):
    ...
    if item["type"] != "movie" and nw_it.lower() in profil.get("studios_fetiches", set()):
        ...
        tags.append(("🏢 Studio fétiche", f"TES notes moyennes ≥ 7,5/10 sur ≥ 2 séries {nw_it} · +4 pts"))
...
def page_quoi_regarder(utz):
    ...
    with cf5:
        f_network = st.selectbox("🏢 Studio (séries)", ["Tous les studios"] + sorted(tous_networks), key="qr_network",
                                 help="Le network (studio/chaîne) déjà présent dans tes données — 0 appel. Les films n'ont pas de studio Trakt gratuit : ils sortent du filtre.")
    with cf6:
    ...
    with col_roul3:
        if st.button("🔥 Tendances — le monde regarde", use_container_width=True, key="btn_roulette_tendances",
                     help="2 requêtes max au clic (cache 6 h ensuite) : le top films + séries du moment sur Trakt, hors vus et hors listes."):
